...
const NeuronColorLegend: React.FC = () => (
  <Paper sx={{ p: 2, mt: 2 }}>
    <Typography variant="h6" gutterBottom>Neuron Color Guide</Typography>
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
        ...
        <Typography>High Activation (1.0)</Typography>
      </Box>
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
        <Box sx={{ 
          width: 20, 
        ...
        <Typography>Medium Activation (0.5)</Typography>
      </Box>
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
        <Box sx={{ 
          width: 20, 
        ...
        <Typography>Low Activation (0.0)</Typography>
      </Box>
    </Box>
  </Paper>
);
...
export const NeuronViz: React.FC<NeuronVizProps> = ({
  ...
  const getNeuronColor = (layerIndex: number, neuronIndex: number) => {
    if (animationState.currentLayer === layerIndex) {
      const activation = animationState.signal[neuronIndex] || 0;
      return `rgba(${activation * 255}, ${activation * 100}, ${(1 - activation) * 255}, 1)`;
    }
    const activation = activations?.[layerIndex]?.[neuronIndex] || 0;
    return `rgba(${activation * 255}, ${activation * 100}, ${(1 - activation) * 255}, 0.8)`;
  };
  ...
  return (
    <Box sx={{ mt: 4 }}>
      ...
      {/* Add color legend before the network visualization */}
      <NeuronColorLegend />
