...
export class WeatherTrainer implements ITrainer {
  ...
  constructor() {
    ...
    this.network.train(weatherData.training, {
      iterations: 1,
      errorThresh: 0.01,
      log: false,
      learningRate: 0.01
  ...
  async train(options: TrainingOptions): Promise<void> {
    try {
      ...
      await this.network.trainAsync(weatherData.training, {
  ...
  getPerformanceMetrics(): PerformanceMetrics {
    ...
    weatherData.training.forEach(data => {
    ...
    const total = weatherData.training.length;
